...
public class DateUtils {
  private static Pattern timePattern = Pattern.compile("(?:([0-9]+)y[a-z]*[,\\s]*)?" + "(?:([0-9]+)mo[a-z]*[,\\s]*)?" + "(?:([0-9]+)w[a-z]*[,\\s]*)?" + "(?:([0-9]+)d[a-z]*[,\\s]*)?" + "(?:([0-9]+)h[a-z]*[,\\s]*)?" + "(?:([0-9]+)m[a-z]*[,\\s]*)?" + "(?:([0-9]+)(?:s[a-z]*)?)?", Pattern.CASE_INSENSITIVE);
  ...
  public static LocalDateTime parseDateDiff(String time, boolean future) {
  ...
  public static String formatDateDiff(LocalDateTime date) {
    Calendar c = GregorianCalendar.from(date.atZone(ZoneId.systemDefault()));
  ...
  public static String formatDateDiff(Calendar fromDate, Calendar toDate) {
    ...
    String[] names = new String[]{"år", "år", "månad", "månader", "dag", "dagar", "timme", "timmar", "minut", "minuter", "sekund", "sekunder"};
    int accuracy = 0;
